...
Farmer John 记住了 Q 对 (a_j, h_j)，表示奶牛 h_j 是前 a_j 头奶牛中第一个分数严格更高的。需要求最小字典序的分数序列。

### 核心思路

贪心构造：初始化所有已知分数，遗忘的设为 1。然后按对 (a, h) 排序后依次处理，确保 c[h] > max(c[1..a])，同时检查中间位置的约束。

### 算法流程

1. 读入已知分数，遗忘的设为 1
2. 将所有约束对按 a 排序
3. 对每个约束 (a, h)：计算 max(c[1..a])，确保 c[h] > 该值
4. 检查位置 a+1 到 h-1 的值不超过 max(c[1..a])
5. 若任何值超出 C，输出 -1

### 复杂度分析

- 时间复杂度：O(Q·N)（朴素），可通过排序优化
- 空间复杂度：O(N)

### 关键技巧

- 每个约束对定义了一个"最大值严格递增"的位置
- 遗忘值设为 1 保证字典序最小