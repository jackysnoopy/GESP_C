...
给定 n 个整数和模数 M，对每个 x 求代价 = Σmin((a_i-x) mod M, M-(a_i-x) mod M)，输出最小代价。

### 核心思路

**环上中位数**：将 a_i % M 视为环上 n 个点，代价即 x 到所有点的环上距离和。最小化此和等价于找环上中位数。将值复制 +M 后用滑动窗口 + 前缀和求解。

### 算法流程

1. 计算 r[i] = a[i] % M 并排序
2. 复制数组（每个值 +M），计算前缀和
3. 对每个长度 n 的窗口计算到中位数的距离和
4. 取最小值
