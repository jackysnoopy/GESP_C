...
给定数组 a，构造 b 的过程：反复从 a 头部取出元素，可选择是否放入 b。求字典序最大的 b。允许先移动一个元素到更前的位置。

### 核心思路

**贪心构造 + 枚举移动**：不移动时，b 的最优构造是保留每个 a[i]（当 a[i] ≥ 后缀最大值时）。移动操作可将更大元素提前，改善 b 的字典序。枚举最佳移动方案并比较。

### 算法流程

1. 计算后缀最大值，不移动时的最优 b
2. 尝试将全局最大值移到位置 0，计算新 b
3. 若 b[0] 已是全局最大，尝试改善 b[1]（找被跳过的大元素）
4. 取所有候选 b 中字典序最大的
