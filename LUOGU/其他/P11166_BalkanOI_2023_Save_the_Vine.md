...
Save the Vine!：n 个敌人（臭度 a[i], 绿度 b[i], 丑度 c[i]），你的力量 X 和紫度 Y 固定。击败敌人需 X≥a[i] 且 Y≥b[i]，击败后 X+=c[i]。求击败 k 人的最小 X+Y。

### 核心思路

二分答案 + 贪心：二分 X+Y 的总值，对每个 Y 候选值（取自 b[i]），贪心按 a[i] 排序击败敌人。

### 算法流程

1. 收集所有可能的 Y 值（即所有 b[i]）
2. 二分答案 total = X + Y
3. 对每个候选 Y：X = total - Y，收集 b[i]≤Y 的敌人
4. 按 a[i] 排序，贪心击败（X ≥ a[i] 时 X += c[i]）
5. 若击败 ≥ k 人则可行

### 复杂度分析

- 时间复杂度：O(N·log(N)·log(V))，V 为值域
- 空间复杂度：O(N)

### 关键技巧

- Y 只需取 b[i] 的值（离散化）
- 按 a[i] 贪心是正确的：击败更多人获得更多力量