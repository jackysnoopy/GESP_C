...
将 $n$ 个元素分组，每组内元素值连续且不重复（即组内为 $v, v+1, v+2, \ldots$ 的子序列）。目标是**最大化最小组的人数**。

### 核心思路

**贪心算法**：排序后按值依次处理，维护当前开放的组（等待下一个值加入）。

对于值 $v$（出现 $cnt$ 次）：
- 若 $cnt \ge$ 开放组数：所有组都能扩展，剩余元素新建组。
- 若 $cnt <$ 开放组数：部分组无法扩展，被迫结束。为最大化最小组，**优先结束最长的组**。

使用 `multiset` 存储各组大小（懒标记 offset），结束时取出最大值。
...
2. 按值依次处理（相同值合并计数）：
   - 若值不连续，所有开放组结束，更新答案。
   - 若有开放组，扩展（offset++）。
   - 若 $cnt \ge$ 开放组数：新建 $cnt - open$ 个组。
   - 若 $cnt <$ 开放组数：结束 $open - cnt$ 个最长组，更新答案。
3. 处理剩余开放组，输出答案。

### 复杂度分析

- **时间复杂度**：$O(n \log n)$，排序 + multiset 操作。
- **空间复杂度**：$O(n)$。
