...
### 问题分析
模拟打歌活动获取pt，LP自动恢复，升级恢复LP。用loveca补充LP，求最少loveca数。

### 核心思路
枚举loveca数量（从0开始递增），模拟打歌过程直到LP不足或pt达标。由于参数范围小，loveca不会太大。

### 算法流程
1. 初始LP=d+x+loveca*c
2. 循环打歌：LP-=b, pt+=a, exp+=g
3. 经验满则升级：LP+=lpmax+h
4. 直到pt≥y或LP不足

### 复杂度分析
时间O(loveca_max * songs_per_loveca)，参数范围小可接受。
