...
In Sample Case #1, there is only one person, and they are moving north from $(5, 5)$. This means that all street corners with $y \geqslant 6$ are possible locations for the crepe cart. Of those possibilities, we choose the one with lowest $x \geq 0$ and then lowest $y \geq 6$.
...
### 问题分析
在曼哈顿网格中，每个人朝一个方向移动，他们能够到达的交点是该方向上的所有交点。需要找到被最多人能够到达的交点。

### 核心思路
使用二维差分数组。对于每个人，根据其移动方向，标记其能够到达的区域。最后通过二维前缀和找出被最多人能够到达的交点。

### 算法流程
1. 对于每个测试用例，初始化差分数组
2. 对于每个人，根据其移动方向（N/S/E/W），计算其能够到达的区域
3. 使用坐标变换：$u = x + y$，$v = x - y + Q$，将曼哈顿距离转化为更易处理的形式
4. 在差分数组上进行标记
5. 计算二维前缀和，找到最大值对应的坐标
6. 按照题目要求的优先级（先最小 $x$，再最小 $y$）输出结果

### 复杂度分析
- 时间复杂度：$O(T \times (P + Q^2))$
- 空间复杂度：$O(Q^2)$
