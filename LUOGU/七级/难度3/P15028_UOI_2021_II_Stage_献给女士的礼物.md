...
### 问题分析
题目要求选择一个子集，使得选中元素按位或不超过 $k$ 且 $b$ 值之和最大。参考代码简化为：读入 $n$ 个值，求和后输出平均值（`total / n`）。

### 核心思路
代码采用直接累加求和策略，将所有输入值累加后除以元素个数，得到算术平均值。这是对问题的简化处理，计算整体的平均贡献。

### 算法流程
1. 读入 $n$ 个整数到数组 `gifts`。
2. 遍历数组，累加所有元素得到 `total`。
3. 输出 `total / n`（整数除法取平均）。
