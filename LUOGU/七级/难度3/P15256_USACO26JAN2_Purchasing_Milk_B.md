...
### 问题分析
题目描述了购买牛奶的多重查询场景，但参考代码实际实现的是一个**贪心背包**问题：给定预算 $b$ 和 $n$ 个物品的价格，求最多能购买多少个物品。

### 核心思路
经典的贪心策略：优先购买价格最低的物品。将价格从小到大排序后，依次尝试购买，只要预算足够就购买该物品。这是贪心选择最优性的直接应用——每一步都选择当前最便宜的物品，能最大化购买数量。

### 算法流程
1. 读入物品数量 $n$ 和预算 $b$。
2. 读入 $n$ 个物品的价格。
3. 将价格从小到大排序。
4. 遍历排序后的价格，若当前价格 $\leq$ 剩余预算，则购买（扣减预算，计数器加一）。
5. 输出最终购买数量。

### 复杂度分析
- 时间复杂度：$O(n \log n)$，主要消耗在排序上；贪心遍历为 $O(n)$。
- 空间复杂度：$O(n)$，用于存储价格数组。
