...
### 问题分析
题目要求在广告时段中选择滑雪时间，使得错过节目内容最少。但参考代码实现的是计算数组中最大值与最小值的差。

### 核心思路
代码读取 n 个整数后，使用标准库函数分别找出数组中的最大值和最小值，然后输出两者的差值。这是一种计算数组极差的简单方法。

### 算法流程
1. 读取整数 n
2. 读取 n 个整数到数组 a 中
3. 使用 max_element 找出最大值
4. 使用 min_element 找出最小值
5. 输出 maxVal - minVal

### 复杂度分析
- 时间复杂度：O(n)，遍历数组找最大最小值
- 空间复杂度：O(n)，用于存储数组
