...
题目描述了一个铁路旅行问题：有 $N$ 个城市的铁路线，每段铁路有两种付费方式——纸质车票（费用 $A_i$）和 IC 卡（费用 $B_i$，但需预先购买 IC 卡，费用 $C_i$）。给定旅行路线 $P_1, P_2, \dots, P_M$，需要最小化总费用（IC 卡购买费用 + 乘车费用之和）。IC 卡一旦购买可重复使用，且 $B_i < A_i$。

### 核心思路

参考代码采用了一种简化的计算策略：读入站点序列后，计算所有相邻站点之间距离的绝对值之和并输出。这对应于对每段旅程都计算路径上的总移动距离。

对于完整的解法，核心思路是：对于每段行程（从 $P_j$ 到 $P_{j+1}$），统计途经每条铁路的次数。对于每条铁路 $i$，如果被经过多次，则购买 IC 卡更划算（节省 $A_i - B_i$ 每次，购买成本为 $C_i$）；如果只经过一次，则比较 $A_i$ 和 $B_i + C_i$ 取最小值。
