...
### 问题分析
题目要求在图中从起点 $s$ 到终点 $t$ 的最短路径，有两种操作：沿边移动和通过 MEX 门传送。MEX 门可以将当前位置传送到未访问过的最小编号城市。

### 核心思路
1. 首先从终点 $t$ 进行 BFS，计算每个节点到 $t$ 的最短距离
2. 对于每个节点 $u$，计算通过 MEX 门到达 $u$ 的代价：如果 $u = s$ 代价为 0；如果 $u < s$ 代价为 $u+1$；否则代价为 $u$
3. 答案为 $\min(mexCost(t), \min_{u}(mexCost(u) + distT[u]))$

### 算法流程
1. 读入测试数据，构建邻接表
2. 从终点 $t$ 开始 BFS，计算每个节点到 $t$ 的最短距离
3. 定义 MEX 代价函数 $mexCost(u)$
4. 遍历所有节点，计算 $mexCost(u) + distT[u]$ 的最小值
5. 输出答案

### 复杂度分析
- 时间复杂度：$O(n + m)$，BFS 的时间复杂度
- 空间复杂度：$O(n + m)$，存储图和距离数组

## 解题思路

### 问题分析
给定一个无向图，从起点 $s$ 出发到达终点 $t$。每次操作可以选择：(1) 沿边走到相邻节点；(2) 通过 MEX 门传送到 $\operatorname{mex}(S)$（已访问节点编号集合的最小未出现非负整数）。要求最小操作次数。

### 核心思路
观察 MEX 门的性质：从起点 $s$ 出发，已访问集合初始为 $\{s\}$。若使用 MEX 门到达编号 $u$，需要访问 $\{0, 1, \ldots, u-1\}$ 中所有缺失的编号，因此到达 $u$ 的 MEX 代价为：若 $u = s$ 则代价 $0$；若 $u < s$ 则代价 $u+1$（需要访问 $0$ 到 $u-1$ 共 $u$ 个节点）；若 $u > s$ 则代价 $u$（需要访问除 $s$ 外的 $0$ 到 $u-1$ 中缺失的编号）。最终答案为：先通过 MEX 门到达某个节点 $u$，再通过边从 $u$ 走到 $t$，枚举所有 $u$ 取最小值。

### 算法流程
1. 对每组测试数据，读入图的邻接表
2. 若 $s = t$，直接输出 $0$
3. 从终点 $t$ 做 BFS，计算每个节点到 $t$ 的最短距离 `distT`
4. 定义 `mexCost(u)` 函数：返回通过 MEX 门到达节点 $u$ 的代价
5. 枚举所有节点 $u$，若 $u$ 可达 $t$（`distT[u] != -1`），则用 `mexCost(u) + distT[u]` 更新答案
6. 同时考虑直接通过 MEX 门到达 $t$ 的情况作为初始答案

### 复杂度分析
- 时间复杂度：$O(n + m)$ 每组数据，BFS 为 $O(n + m)$，枚举为 $O(n)$
