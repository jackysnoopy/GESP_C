...
### 问题分析
有 N 种颜色的团子，第 i 种有 A_i 个。每串团子需要 3 个团子，且任意两个团子的颜色差不超过 1。目标是最大化制作的团子串数。

### 核心思路
参考代码的逻辑是：遍历所有颜色，找到拥有数量最多的那种颜色的团子数，直接输出。其思想是：如果某种颜色有足够多的团子，可以全部用同色团子组成串（同色团子两两差为 0，满足约束）。答案至少是 max(A_i) / 3 的上界，而代码直接取 max(A_i) 作为输出。
