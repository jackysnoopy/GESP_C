...
题目要求判断能否从数列中选择若干个数（非空子集），使得这些数的最小公倍数恰好等于给定的定值 $k$。关键约束包括：$n$ 最大为 $10^5$，$k$ 最大为 $10^{18}$，$a_i$ 最大为 $10^9$。需要高效地判断是否存在这样的子集。

### 核心思路

参考代码采用的策略是计算整个数组所有元素的 LCM，然后输出结果。具体地，代码通过逐个遍历数组元素，利用公式 $\text{lcm}(a, b) = a / \gcd(a, b) \times b$ 依次累积求出全部元素的最小公倍数。先除后乘的计算方式是为了避免中间结果溢出。

对于判断 LCM 是否等于 $k$ 的完整解法，其核心思想是：将 $k$ 进行质因数分解，对于 $k$ 的每个质因子 $p^e$，需要在数列中找到至少一个数，使其包含 $p$ 的幂次恰好为 $e$。如果每个质因子都能被满足，则答案为 `Yes`，否则为 `No`。

### 算法流程

1. 读入数列长度 $n$ 和各元素 $a_i$
2. 初始化 $\text{lcm} = 1$
3. 遍历每个元素 $x$，执行 $\text{lcm} = \text{lcm} / \gcd(\text{lcm}, x) \times x$
4. 输出最终的 LCM 值

### 复杂度分析

- **时间复杂度**：$O(n \log M)$，其中 $M$ 为数组元素的最大值，每次 $\gcd$ 运算的时间为 $O(\log M)$
- **空间复杂度**：$O(n)$，用于存储输入数组
...
题目要求判断能否从数列中选择若干个数（非空子集），使得这些数的最小公倍数（LCM）恰好等于给定的定值 $k$。关键约束包括：$n$ 最大为 $10^5$，$k$ 最大为 $10^{18}$，$a_i$ 最大为 $10^9$。需要高效地判断是否存在这样的子集。

### 核心思路

参考代码采用的策略是计算整个数组所有元素的 LCM，然后输出结果。具体地，代码通过逐个遍历数组元素，利用公式 $\text{lcm}(a, b) = \frac{a}{\gcd(a, b)} \times b$ 依次累积求出全部元素的最小公倍数。先除后乘的计算方式是为了避免中间结果溢出。
