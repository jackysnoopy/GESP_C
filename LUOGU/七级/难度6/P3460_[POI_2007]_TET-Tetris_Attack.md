...
这是一个栈+模拟问题。需要通过交换相邻元素来消除所有匹配的符号对。

### 核心思路

1. 从栈底开始扫描
2. 如果当前元素和下一个元素匹配，直接消除
3. 否则，找到匹配的元素，通过交换使其相邻

### 算法流程

1. 读取栈的内容
2. 从栈底开始扫描
3. 如果当前元素和下一个元素匹配，消除它们
4. 否则，找到匹配的元素，交换使其相邻
5. 记录交换操作
...
int main() {
    ...
    for (int i = 0; i < 2 * n; i++) cin >> stack[i];
    
    vector<int> ans;
    for (int i = 0; i < 2 * n - 1; i++) {
        if (stack[i] == stack[i + 1]) {
            stack.erase(stack.begin() + i, stack.begin() + i + 2);
            i -= 2;
        }
    }
