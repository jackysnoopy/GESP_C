...
在 $N$ 个路口、$M$ 条道路的图中，每个路口有一盏高度为 $H_i$ 的路灯。每条道路长度为 $L_j$，可以放倒路灯使其平铺在道路上，但路灯高度不能超过道路长度。每条道路可以放多盏路灯，只要总高度不超过道路长度。每盏路灯至多使用一次。目标是最大化被放倒（借用）的路灯数量。

### 核心思路

采用贪心策略：优先处理较高的路灯。将所有路口按路灯高度从大到小排序，依次为每盏路灯选择一条与之相连且剩余容量足够的道路，优先选择剩余容量最大的道路（以保留更多容量给后续路灯）。这种"高灯优先、最优匹配"的策略能最大化路灯放置数量。

### 算法流程

1. 读入路口数 $N$、道路数 $M$，路灯高度 $H$，道路连接关系 $A_j, B_j$ 和长度 $L_j$。
2. 初始化每条道路的剩余容量 $road\_cap = L$。
3. 将路口按路灯高度从大到小排序。
4. 依次处理每个路口 $i$：
   - 在所有与路口 $i$ 相连（$A_j = i$ 或 $B_j = i$）且剩余容量 $\ge H_i$ 的道路中，选择剩余容量最大的那条。
   - 若找到这样的道路，将其容量减去 $H_i$，计数器 $ans$ 加 1。
5. 输出 $ans$。

### 复杂度分析

- **时间复杂度**：$O(N \log N + N \cdot M)$。排序 $O(N \log N)$，对每个路口遍历所有道路 $O(N \cdot M)$，在 $N, M \le 10^6$ 时此暴力匹配效率较低。
- **空间复杂度**：$O(N + M)$，存储路灯高度和道路信息。

> **注意**：此为参考代码中的暴力解法。对于全范围数据，需要更高效的数据结构（如并查集或按边分组的链表）来加速"找到与节点 $i$ 相连的、容量足够的最大容量道路"这一查询。
