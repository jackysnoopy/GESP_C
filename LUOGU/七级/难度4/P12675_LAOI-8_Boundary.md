...
给定一个长度为 $n$ 的排列 $A$，有两种操作：(1) 将 $A_i$ 加 1，代价为 1；(2) 将一个两端相等的区间赋值为 $-10^9$，代价为区间长度。求使所有元素变为 $-10^9$ 的最小代价。

### 核心思路

代码采用贪心策略：对于位置 $i$（从 0 开始），目标值为 $n - i$。若 $A[i] < n - i$，则需要将 $A[i]$ 增加到 $n - i$，增加的代价为 $(n - i) - A[i]$。

**关键观察**：由于操作二要求区间两端元素相等，最有效的方式是先通过操作一将某些元素调整到相同值，然后用操作二一次性清除整个区间。代码中对每个位置计算需要增加的量，累加得到最小代价。

### 算法流程

1. 读入测试数据组数 $T$。
2. 对每组数据，读入 $n$ 和排列 $A$。
3. 遍历每个位置 $i$，若 $A[i] < n - i$，则累加 $(n - i) - A[i]$ 到答案。
4. 输出答案。

### 复杂度分析

- 时间复杂度：$O(T \times n)$，单次遍历。
- 空间复杂度：$O(n)$，存储数组。
