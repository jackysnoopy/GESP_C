...
### 问题分析
Dante 被困在 N×N 的螺旋编号房间中，从房间 1 出发到达中心房间 N²。每次只能移动到编号更大且相邻的房间，要求恰好用 K 步到达（K < N²-1）。每次跳跃到非连续编号的房间算作"捷径"，需要找到可行方案或判断不可能。

### 核心思路
贪心策略：从当前位置出发，尽可能走捷径（跳到最远的可达位置 cur + 2(N-1)），但捷径次数不能超过允许的上限。允许的捷径次数上限由公式 `K - (total - 1 - K)` 决定，其中 total-1-K 是必须保留的"省步数"。

### 算法流程
1. 若 K ≥ total-1（total = N²），直接输出 IMPOSSIBLE
2. 从当前位置 cur = 1 开始，计算每步可达的最远位置 max_next = min(total, cur + 2(N-1))
3. 若 max_next > cur+1 且当前捷径次数未超上限，则跳跃到 max_next（走捷径），否则走一步到 cur+1
4. 重复直到到达 total，输出使用的捷径次数

### 复杂度分析
- 时间复杂度：O(N²)，每步至少前进 1，最多走 N²-1 步
- 空间复杂度：O(1)，仅使用常数额外空间
