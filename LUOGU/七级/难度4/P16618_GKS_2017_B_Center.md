...
给定平面上 $N$ 个带权重点，找一个中心点 $(X, Y)$ 使得到所有点的切比雪夫距离加权和最小，即 $\sum \max(|X-X_i|, |Y-Y_i|) \times W_i$ 最小。

### 核心思路

使用迭代优化的方法（类似 Weiszfeld 算法的变体）：

- 切比雪夫距离可以转化为：$\max(|dx|, |dy|)$
- 通过迭代更新中心点位置，逐步逼近最优解
- 每次迭代根据当前中心点的位置，将点分为两类并重新计算加权中心

### 算法流程

1. 初始化中心点为 $(0, 0)$
2. 迭代 100 次：
   - 对于每个点，计算与当前中心的切比雪夫距离
   - 根据距离关系重新计算加权中心
   - 更新中心点位置
3. 计算最终的加权距离和
...
- 时间复杂度：$O(T \cdot N \cdot \text{iter})$，其中 iter=100
- 空间复杂度：$O(N)$
