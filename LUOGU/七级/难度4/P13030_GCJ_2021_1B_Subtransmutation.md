...
### 问题分析
给定一种炼金术法术：消耗一单位金属 i，可生成一单位金属 (i-A) 和一单位金属 (i-B)（若编号不为正则不生成）。初始只有一个矿工提供一单位任意金属，需要用它分解出足够的金属 1~N（分别需要 U1~UN 个单位），求最小的起始金属编号。

### 核心思路
暴力枚举起始金属编号（从 N 到 1000），对每个起始值模拟分解过程：从大到小遍历金属编号，每种金属若存在则按规则分解到更小的编号，最终检查是否满足所有需求。

### 算法流程
1. 从 start = N 开始向上枚举起始金属编号
2. 初始化 have[start] = 1
3. 从 i = start 到 1 逆序遍历：
   - 若 i ≤ A，该金属被消耗但不生成任何东西
   - 若 A < i ≤ B，生成 have[i] 个金属 (i-A)
   - 若 i > B，生成 have[i] 个金属 (i-A) 和 have[i] 个金属 (i-B)
4. 检查 1~N 号金属是否都满足需求
5. 找到第一个满足的 start 即为答案；若到 1000 都不行则输出 IMPOSSIBLE

### 复杂度分析
- 时间复杂度：O(T × 1000 × 1000)，枚举起始值最多 1000 次，每次模拟最多遍历 1000 种金属
- 空间复杂度：O(1000)，存储每种金属的数量
