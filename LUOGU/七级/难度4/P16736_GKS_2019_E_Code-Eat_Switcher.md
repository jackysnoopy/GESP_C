...
有 $S$ 个时间段，每个时间段可以分配比例 $f$ 给编码（获得 $f \times C_i$ 编码量）和 $(1-f)$ 给吃东西（获得 $(1-f) \times E_i$ 吃的量）。对 $D$ 天的需求 $(A_i, B_i)$，判断是否能满足。
...
- 每个时间段可以独立分配比例
- 总编码能力为 $\sum C_i$，总吃东西能力为 $\sum E_i$
- 只要 $A \le \sum C_i$ 且 $B \le \sum E_i$，就一定能满足

因为可以将编码能力强的时间段分配给编码，吃东西能力强的时间段分配给吃东西。
...
2. 计算总编码能力和总吃东西能力
3. 对于每天的需求 $(A, B)$：
   - 如果 $A \le \text{总编码能力}$ 且 $B \le \text{总吃东西能力}$，输出 Y
   - 否则输出 N
