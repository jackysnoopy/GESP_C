...
给定 N 个水源，每个水源有流速 R_i 和温度 C_i。目标是将泳池注满体积恰好为 V、温度恰好为 X 的水，求最短时间。如果无法达成目标温度，输出 IMPOSSIBLE。

### 核心思路

本题的核心是：当所有水源同时开启时，混合水的温度等于各水源流速加权的温度平均值。因此，判断能否达到目标温度，只需检查所有水源混合后的加权平均温度是否等于 X。若等于 X，则最优策略是同时开启所有水源，用 V / total_rate 秒注满。

### 算法流程

1. 读入所有水源的流速 R_i 和温度 C_i。
2. 计算总流速 total_rate = ΣR_i 和加权温度和 weighted_sum = Σ(R_i × C_i)。
3. 若 total_rate 为 0，或加权平均温度 weighted_sum / total_rate 与目标温度 X 的差超过 1e-9，则输出 IMPOSSIBLE。
4. 否则，最短时间为 V / total_rate，输出该值。

### 复杂度分析

- 时间复杂度：O(T × N)，其中 T 为测试用例数，N 为水源数。每个用例只需遍历一次水源。
- 空间复杂度：O(N)，用于存储各水源的流速和温度。
