...
石头剪刀布游戏，对手出牌已知。我方可选择出牌，但换牌需扣分。目标是最大化总得分。

### 核心思路

动态规划：$dp[j][t]$ 表示当前轮出牌 $j$（0/1/2），已换牌 $t$ 次时的最大得分。每轮考虑不换牌或换到其他两种牌。

### 算法流程

1. 定义得分函数：平局得 $a_i$，赢了得 $2a_i$，输了得 $0$
2. 初始化第一轮：对三种出牌分别计算得分
3. 从第二轮开始 DP 转移：
   - 不换牌：$ndp[j][t] = dp[j][t] + score(i, j)$
   - 换牌到 $k$：$ndp[k][t+1] = dp[j][t] + score(i, k) - b[t]$
4. 答案为所有状态中的最大值

### 复杂度分析

- 时间复杂度：$O(N^2)$，每轮需要枚举起始出牌和换牌次数
- 空间复杂度：$O(N)$，使用滚动数组
