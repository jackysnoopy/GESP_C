...
### 问题分析
在有向图中，每条边有唯一颜色。求从 $S$ 到 $T$ 的所有行走（长度不超过 $10^{100}$）中，颜色序列字典序最小的那个。

### 核心思路
由于每条边颜色唯一，将边权设为颜色值，求 $S$ 到 $T$ 的最短路径（颜色值之和最小）即等价于求字典序最小的颜色序列。这是因为 Dijkstra 按距离从小到大扩展，最先到达 $T$ 的路径的颜色序列在所有路径中字典序最小。

### 算法流程
1. 建立邻接表，边权为颜色值
2. 从 $S$ 开始运行 Dijkstra 算法，记录每个节点的前驱和对应边的颜色
3. 若 $T$ 不可达，输出 "IMPOSSIBLE"
4. 从 $T$ 回溯到 $S$，收集路径上的颜色序列
5. 若序列长度超过 $10^6$，输出 "TOO LONG"，否则输出序列

### 复杂度分析
- **时间复杂度**：$O((N + M) \log N)$，Dijkstra 标准复杂度
