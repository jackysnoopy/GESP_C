...
### 问题分析
给定N种食材，每种食材有P个包裹。每个包裹的重量与标准重量R[i]的比值需要在[90%, 110%]范围内。每个套餐需要包含每种食材的一个包裹，求最多能组成多少个有效套餐。

### 核心思路
贪心匹配策略。对于每种食材的包裹按重量排序，然后尝试将各食材的最小包裹组合成套餐。如果当前组合有效（所有食材的份量范围有交集），则形成一个套餐；否则跳过当前最小的包裹。

### 算法流程
1. 读入N、P和各食材的标准重量R[i]
2. 读入每种食材的P个包裹重量Q[i][j]
3. 对每种食材的包裹按重量排序
4. 初始化各食材的索引idx[i] = 0
5. 循环直到无法继续：
   - 对于每种食材i，计算当前包裹能组成的有效份量范围：
     - low = ceil(q * 10 / (9 * R[i]))（最小份数）
     - high = floor(q * 10 / (11 * R[i]))（最大份数）
   - 计算所有食材的份量范围交集：min_serving = max(low), max_serving = min(high)
   - 如果交集有效（min_serving ≤ max_serving）：
     - ans++，所有食材索引+1（使用当前包裹）
   - 否则：
     - 找到当前包裹重量最小的食材，跳过其最小包裹（idx[min_idx]++）
6. 输出ans

### 复杂度分析
- 时间复杂度：$O(N \times P \times \log P)$，排序$O(P \log P)$，主循环最多$O(N \times P)$次
- 空间复杂度：$O(N \times P)$，存储包裹重量
