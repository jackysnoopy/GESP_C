...
### 问题分析
给定数组 A 和上限 M，求最大的非负整数 k，使得 Σ(A[i] xor k) ≤ M。

### 核心思路
暴力枚举 k 从 0 到 1000000，对每个 k 计算所有 A[i] xor k 的和。由于 sum 随 k 增大单调不减（从某点开始），找到第一个 sum > M 的 k 即可停止。当前代码为简化实现，仅适用于小数据范围。

### 算法流程
1. 对每个测试用例，读入 N、M 和数组 A
2. 从 k = 0 开始枚举：
   - 计算 sum = Σ(A[i] xor k)
   - 若 sum ≤ M，更新答案为 k
   - 若 sum > M，停止枚举
