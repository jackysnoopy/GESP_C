# [GESP202412 七级] 燃烧

## 题目描述

对应的选择、判断题：

小杨有一棵包含 $n$ 个节点的树，其中节点的编号从 $1$ 到 $n$。节点 $i$ 的权值为 $a_i$。

小杨可以选择一个初始节点引燃，每个燃烧的节点会将其相邻节点中权值**严格小于**自身权值的在节点间扩散直到不会有新的节点被引燃。

小杨想知道在合理选择初始节点的情况下，最多可以燃烧多少个节点。

## 输入格式

第一行包含一个正整数 $n$，表示节点数量。

第二行包含 $n$ 个正整数 $a_1,a_2,\dots,a_n$，代表节点权值。

之后 $n-1$ 行，每行包含两个正整数 $u_i,v_i$，代表存在一条连接节点 $u_i$ 和 $v_i$ 的边。

## 输出格式

输出一个正整数，代表最多燃烧的节点个数。

## 样例

### 样例输入 1
```
5
6 2 3 4 5
1 2
2 3
2 5
1 4
```

### 样例输出 1
```
3
```

## 提示

| 子任务编号 | 数据点占比 |     $n$     |
| :--------: | :--------: | :---------: |
|    $1$     |   $20\%$   |  $\leq 10$  |
|    $2$     |   $20\%$   | $\leq 100$  |
|    $3$     |   $60\%$   | $\leq 10^5$ |

对于全部数据，保证有 $1\leq n\leq 10^5$，$1\leq a_i\leq 10^6$。

## 参考代码

```cpp
#include <iostream>
#include <vector>
#include <algorithm>
using namespace std;

int main() {
    ios::sync_with_stdio(false);
    cin.tie(nullptr);
    int n;
    cin >> n;
    vector<int> a(n + 1);
    for (int i = 1; i <= n; i++) cin >> a[i];
    vector<vector<int>> g(n + 1);
    for (int i = 0; i < n - 1; i++) {
        int u, v;
        cin >> u >> v;
        g[u].push_back(v);
        g[v].push_back(u);
    }
    vector<int> order(n);
    for (int i = 0; i < n; i++) order[i] = i + 1;
    sort(order.begin(), order.end(), [&](int x, int y) { return a[x] < a[y]; });
    vector<long long> f(n + 1, 1);
    for (int u : order) {
        for (int v : g[u]) {
            if (a[v] < a[u]) f[u] += f[v];
        }
    }
    long long ans = 0;
    for (int i = 1; i <= n; i++) ans = max(ans, f[i]);
    cout << ans << "\n";
    return 0;
}
```

## 解题思路

### 问题分析

给定一棵树，节点有权值。从某节点引燃，火焰会扩散到相邻且权值严格小于当前节点的节点。求选择最优初始节点时最多能燃烧多少节点。

### 核心思路

按权值从小到大排序处理节点。对每个节点 $u$，累加其所有邻居中权值小于 $a[u]$ 的节点的燃烧范围 $f[v]$。这样 $f[u]$ 表示以 $u$ 为起点能燃烧的节点数。

**关键性质**：权值小的节点先被处理，保证了 DP 转移的正确性——当处理 $u$ 时，所有 $a[v] < a[u]$ 的邻居 $v$ 的 $f[v]$ 已经计算完毕。

### 算法流程

1. 读入树的结构和节点权值。
2. 按权值从小到大排序所有节点。
3. 对每个节点 $u$，初始化 $f[u] = 1$，累加所有 $a[v] < a[u]$ 的邻居的 $f[v]$。
4. 输出所有 $f[i]$ 的最大值。

### 复杂度分析

- 时间复杂度：$O(n \log n)$，排序主导。
- 空间复杂度：$O(n)$，存储树和 DP 数组。
