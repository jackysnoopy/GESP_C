...
Alice 和 Bob 博弈。Alice 选非空子集 $x$，Bob 选子集 $y$ 合并后排列。Bob 胜当且仅当能排成回文串。Alice 想赢。

### 核心思路

回文串条件：至多一个字符出现奇数次。Alice 需要选择一个子集 $x$，使得 $x + Y$ 中奇数次字符数 $> 1$（Bob 无法排成回文）。
...
2. 枚举 $X$ 的子集（位掩码），检查：
   - 子集非空
   - 合并后奇数次字符数 $> 1$
3. 若存在合法子集，输出字典序最小的排列
4. 否则输出 $-1$

### 复杂度分析

- 时间复杂度：$O(2^{26} \cdot 26)$，枚举所有字符子集
- 空间复杂度：$O(26)$
