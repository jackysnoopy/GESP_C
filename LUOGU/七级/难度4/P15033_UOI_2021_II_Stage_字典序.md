...
### 问题分析
给定一个数组，需要重新排列使得相邻元素不同，并且字典序最小。若无法满足条件则输出-1。

### 核心思路
1. 首先检查可行性：如果某个元素出现次数超过(n+1)/2，则无法满足条件
2. 使用贪心策略：每次选择出现次数最多且与前一个不同的元素
3. 使用优先队列维护元素按出现次数排序

### 算法流程
1. 统计每个元素的出现次数
2. 检查最大出现次数是否超过(n+1)/2
3. 使用最大堆存储(出现次数, 元素值)
4. 每次取出堆顶，若与前一个不同则使用，否则取次多的元素
5. 将使用后的元素重新入堆
