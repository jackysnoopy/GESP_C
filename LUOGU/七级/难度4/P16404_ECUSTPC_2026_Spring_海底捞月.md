...
初始分数 $x$，有 4 种招数各可用 $a_i$ 次。求使用招数后的最高分数。

### 核心思路

暴力枚举：由于初始分数较小（$x \leq 100$），招数使用次数有限（$a_i \leq 400$），可以枚举每种招数的使用顺序和次数。

### 算法流程

1. 枚举 4 种招数的使用顺序（$4! = 24$ 种排列）
2. 对每种排列，贪心使用招数以最大化分数
3. 输出所有排列中的最大分数

### 复杂度分析

- 时间复杂度：$O(24 \cdot \sum a_i)$
- 空间复杂度：$O(1)$
