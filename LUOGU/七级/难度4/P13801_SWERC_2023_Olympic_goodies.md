...
### 问题分析
给定一棵 $N$ 个节点的树和 $P$ 件物品，YAOGS 将物品放置在树的边上（每条边可放 0 件或多件）。获胜者选择一条路径收集路径上所有物品。YAOGS 希望放置物品使得获胜者能收集到的最大物品数尽可能小。求在 YAOGS 最优放置策略下，获胜者能收集的最大物品数。

### 核心思路
答案取决于树的直径。树的直径（最长路径的边数）为 $d$，则直径路径上有 $d+1$ 个节点、$d$ 条边。YAOGS 最多在直径路径上放置 $d$ 件物品，获胜者沿直径路径最多收集 $d$ 件。若 $P \leq d$，YAOGS 可将所有物品放在直径路径上，答案为 $P$；若 $P > d$，答案为 $d+1$（考虑节点数）。代码最终输出 $\min(P, \text{diameter}+1)$。

### 算法流程
1. 读入树的边，构建邻接表
2. 从节点 $0$ 开始 BFS，计算每个节点的深度 `depth` 和父节点 `parent`
3. 找到深度最大的节点
4. 从最深节点沿 `parent` 指针回溯到根节点，计算路径长度即为直径 `diameter`
5. 输出 $\min(P, \text{diameter}+1)$

### 复杂度分析
- **时间复杂度**：$O(N)$。BFS 遍历 $O(N)$，找最深节点 $O(N)$，回溯计算直径 $O(N)$
- **空间复杂度**：$O(N)$。存储邻接表、深度数组、父节点数组和 BFS 队列
