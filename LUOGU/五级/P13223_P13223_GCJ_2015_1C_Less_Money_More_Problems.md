...
已有 D 种面额的硬币，每种最多用 C 枚。需添加最少的新面额，使得 1~V 的所有价值都可表示。

### 核心思路

- 贪心维护当前能表示的最大连续范围 [1, maxReachable]。
- 将已有面额排序，依次尝试加入：若下一个面额 ≤ maxReachable + 1，则加入后范围扩展到 maxReachable + C × denom。
- 若下一个面额 > maxReachable + 1，则存在无法表示的值（maxReachable + 1），需添加面额 maxReachable + 1。
- 重复直到 maxReachable ≥ V。

### 算法流程

1. 读入并排序已有面额。
2. 初始化 maxReachable = 0，ans = 0，idx = 0。
3. 当 maxReachable < V 时：
   - 若 idx < D 且 existing[idx] ≤ maxReachable + 1，加入该面额，maxReachable += C × existing[idx]，idx++。
   - 否则，添加新面额 maxReachable + 1，maxReachable += C × (maxReachable + 1)，ans++。
4. 输出 ans。

### 复杂度分析

- 时间复杂度：O(D log D + 新面额数)，排序 O(D log D)，贪心过程 O(D)。
- 空间复杂度：O(D)，存储面额数组。
