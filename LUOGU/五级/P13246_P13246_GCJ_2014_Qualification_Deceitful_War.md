...
Naomi 和 Ken 各有 N 个不同重量的积木。两种模式：
- **War**：Naomi 公布真实重量，Ken 用最优策略应对（用刚好能赢的最小积木，或最小积木输掉）。
- **Deceitful War**：Naomi 知道 Ken 的全部积木，可以报假重量（但天平结果必须与报的一致）。Ken 仍按 War 策略应对。

### 核心思路

- **Deceitful War**：排序后，双指针扫描。Naomi 用最小的积木去"骗" Ken 用掉最大的积木（报一个让 Ken 认为需要出最大积木的值），然后用自己较大的积木赢得后续轮次。统计 naomi[i] > ken[j] 的匹配数。
- **War**：Ken 用最优策略——对 Naomi 的每个积木，用刚好能赢的最小积木。统计 Ken 能赢的次数，Naomi 赢 = N - Ken 赢。

### 算法流程

1. 读入并排序两人的积木重量。
2. Deceitful War：双指针 i=j=0，若 naomi[i] > ken[j] 则两者各进一位并计分，否则仅 naomi 指针进位（用小积木消耗）。
3. War：双指针 i=j=0，若 ken[j] > naomi[i] 则 Ken 赢一轮（两者各进一位），否则 ken 指针进位。Naomi 赢 = N - Ken 赢的轮数。
4. 输出两个分数。
