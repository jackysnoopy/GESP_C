...
You have been assigned an expert miner to assist you. The expert miner can fetch a single unit of any metal you want. From that unit, you can use your spell to create other metals and then use the spell on the resulting metals to create even more units.
...
这道题是一个关于炼金术的问题。我们有一个炼金术公式：给定一个金属$i$，我们可以将其销毁，生成金属$i-A$和金属$i-B$（如果这些值是正整数）。我们的目标是从单个金属开始，通过应用这个公式，最终得到所有需要的金属。

关键观察是，我们可以从任意金属开始，应用公式多次，最终得到我们需要的金属集合。问题是要找到最小的起始金属。

### 算法设计

1. **贪心策略**：从最小的可能金属（1）开始尝试，直到找到一个能满足所有需求的金属。
2. **模拟过程**：对于每个可能的起始金属，模拟炼金术过程：
   - 从起始金属开始，应用公式分解金属
   - 优先满足高编号金属的需求
   - 记录剩余的金属数量
3. **验证**：检查是否所有金属需求都得到满足

### 关键点

- 我们需要考虑金属分解的顺序
- 超出需求的金属可以丢弃
- 如果找不到任何金属能满足需求，则返回"IMPOSSIBLE"

## 算法原理

本题采用贪心算法结合模拟。对于每个可能的起始金属，我们模拟炼金术过程，尝试满足所有金属需求。由于金属编号越大越重，我们应该优先处理大编号金属，将其分解为小编号金属。
...
- **时间复杂度**：O(T * M * N)，其中T是测试用例数量，M是尝试的最大金属编号（500），N是金属种类数量。
- **空间复杂度**：O(N)，用于存储金属需求和当前拥有的金属数量。

## 注意事项

1. 我们需要从最小的可能金属开始尝试，确保找到的是最小的满足条件的金属。
2. 在处理过程中，我们需要考虑金属分解的顺序，优先处理大编号金属。
3. 超出需求的金属可以丢弃，不影响结果。
