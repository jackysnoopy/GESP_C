...
构造一棵高度为 k+2 的树，底层有 n 个黑球和 m 个白球，上层有 a_i 个节点。要求从根出发随机走子节点到达底层，使落在黑球上的概率最大。

### 核心思路

- 最优策略：构造一棵树，使黑球集中在概率最高的叶子位置。
- 计算各层的"分支因子" c_i：主路径分支 c_k=a_k, c_i=a_i-a_{i+1}+1 (i<k), c_0=(n+m)-a_1+1。
- 叶子按概率从高到低排列：非主路径叶子概率 = P_main × 前缀积，主路径叶子概率 = P_main。
- 将黑球优先放在概率最高的叶子上。
...
1. 读入 n, m, k 及 a_i 数组。
2. 计算各层分支因子 c_i。
3. 计算主路径概率 P_main = ∏(1/c_i)。
4. 计算前缀积 prefixProd[i] = ∏(c[0]...c[i-1])。
5. 从最高层到最低层，将黑球优先放在概率最高的非主路径叶子上。
6. 剩余黑球放在主路径叶子上。
7. 总概率 = 各层黑球数 × 对应概率之和，对 998244353 取模。

### 复杂度分析

- 时间复杂度：O(k)，线性扫描各层。
- 空间复杂度：O(k)，存储 a_i、c_i 和前缀积。
